...
public class OutboxEventService {
    ...
    public List<OutboxEvent> claimEvents(){
        ...
        OffsetDateTime claimedUntil = OffsetDateTime.now().plusSeconds(30);
    ...
    public void markAsPublished(UUID eventId){
        ...
        if(updated != 1){
            throw new IllegalStateException("Failed to mark outbox event as published: " + eventId);
        }
    }

    @Transactional
    public void recordFailure(UUID eventId, String error){
        int updated = outboxEventRepository.recordFailure(eventId, error);
        if(updated != 1){
            throw new IllegalStateException("Failed to record failure for outbox event: " + eventId);
        }
    }
}
